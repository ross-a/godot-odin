...
get_godot_strings :: proc() -> ^[dynamic]^godot.String {
  @static godot_strings : ^[dynamic]^godot.String
	// NOTE: im not sure how long some godot.Strings need to stay around, but
	// a str : string `property`, in your struct can cause "malloc_consolidate(): unaligned fastbin chunk detected"
	// if it is free'd too soon  (in the godot editor)
  if godot_strings == nil {
    godot_strings = new([dynamic]^godot.String)
  }
  return godot_strings
}
...
###_new :: proc "c" (p_userdata: rawptr) -> gd.GDExtensionObjectPtr {
  ...
  //fmt.println(#procedure, p_userdata)
  inst := new(###)
...
###_get_func :: proc "c" (p_instance: gd.GDExtensionClassInstancePtr, p_name: gd.GDExtensionConstStringNamePtr, r_ret: gd.GDExtensionVariantPtr) -> gd.GDExtensionBool {
  ...
  if p_instance != nil && slice.contains(names, name) {
    ...
    if fmt.tprintf("%s", sf.type) == "string" {
      str := new(godot.String)
      godot_strings := gdc.get_godot_strings()
      append(godot_strings, str)
...
`, struct_list[idx], name))
                  {
                    ...
                    if ret_type != "" {
                      ...
                      for idx in 0..<arg_len {
                        ...
                        if strings.contains(s, "u128") || strings.contains(s, "i128") do bit_set_size = "u128" // won't work! godot maxes out at 64
...
`, struct_list[idx], name))
      ...
      if strings.has_prefix(strings.trim_left_space(line), "// ### auto-generated unregister_class is put here") {
        ...
        os.write_string(out_fd, "  godot_strings := gdc.get_godot_strings()\n")
        os.write_string(out_fd, "  for c in godot_strings {\n")
        os.write_string(out_fd, "    free(c)\n")
        os.write_string(out_fd, "  }\n")
        os.write_string(out_fd, "  delete(godot_strings^)\n")
        os.write_string(out_fd, "  free(godot_strings)\n")
